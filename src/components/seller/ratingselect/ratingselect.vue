...
export default {
	...
	created(){
		//console.log(this.negatives.length)
	},
	methods:{
		select(type,event){
			...
			//this.$dispatch('ratingtype.select',type)向上派发事件。vue1.0里面可用dispatch和emit.vue2.0只用emit
			this.$emit('select',type)//把ratingselect.vue里面的事件派发到父组件food.vue去。
		},
		toggleContent(event){
			if (!event._constructed) {
          		return;
